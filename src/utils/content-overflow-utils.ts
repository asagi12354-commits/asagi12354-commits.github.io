/**
 * 内容溢出增强：MathJax 显示公式滚动条、宽表格横向滚动容器。
 * 由 Layout.astro 与 Swup 内容替换时调用。
 */

function initCustomScrollbar(): void {
	// MathJax(SVG) 的块级公式是 <mjx-container display="true">。
	// 宽公式超出正文时用浏览器原生横向滚动条，避免撑破布局。
	const mathElements = document.querySelectorAll(
		'mjx-container[display="true"]:not([data-scrollbar-initialized])',
